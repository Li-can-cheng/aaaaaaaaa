    ...
    <el-form ref="form" :model="user" :rules="rules" label-width="120px">
        <el-form-item label="求职类型" prop="workDayPreferenceList">
            <el-checkbox-group v-model="user.workDayPreferenceList" @change="handleCheckedWordDayChange">
                <el-checkbox v-for="workDay in jobtpyelist" :label="workDay.key" :key="workDay.key">
                    {{ workDay.label }}
                </el-checkbox>
            </el-checkbox-group>
        </el-form-item>
            ...
            <el-table :data="user.timeSlotList" style="width: 50%">
                <el-table-column label="城市" prop="city" align="center">
                    <template slot-scope="scope">
                        <el-time-select v-model="scope.row.city" :picker-options="{
                            start: workcity,
                            step: step,
                            end: wordEndTime,
                            minTime: scope.row.startMinTime,
                            maxTime: scope.row.startMaxTime
                        }" placeholder="选择城市" size="mini" @change="startChange(scope.$index)" class="city">
                        </el-time-select>
                    </template>
                </el-table-column>
                <el-table-column align="center">
                    <template slot="header" slot-scope="scope">
                        <el-button type="text" @click="addTimeSlot" icon="el-icon-circle-plus-outline">添加城市</el-button>
                    </template>
                    <template slot-scope="scope">
